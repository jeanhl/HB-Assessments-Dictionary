...
def kids_game(names):
    ...
    word_dict = {} # {word:last letter}
    letter_dict = {} # {first letter: list of words with that letter}
    ...
    while flag in word_dict:
        next_letter = flag[-1]
        possibles = letter_dict[next_letter]
        print possibles
        choosen = random.choice(possibles)
        output.append(choosen)
        if choosen[-1] in letter_dict:
            flag = output[-1]
        else:
            return output
